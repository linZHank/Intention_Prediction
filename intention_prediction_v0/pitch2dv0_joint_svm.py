"""Use Support Vector Machine on  pitch2dv0 joint data"""
...
for i,nf in enumerate(num_frames):
  ...
  classes_test = svm[bki].predict(Xte)
...
dffilename = os.path.join(result_path, "joint_svm_scores.csv")
...
predevenfilename = os.path.join(result_path, "joint_svm_pred_even.txt")
...
preddiscfilename = os.path.join(result_path, "joint_svm_pred_disc.txt")
np.savetxt(preddiscfilename, pred_disc, fmt="%d")
# Save logarithm weighted predictions
predlogrfilename = os.path.join(result_path, "joint_svm_pred_logr.txt")
